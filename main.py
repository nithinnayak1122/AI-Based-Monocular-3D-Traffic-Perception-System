...
def draw_3d_bbox(img, bbox, dist, track_id, cls_name, color, conf):
    ...
    label = f"{cls_name.upper()} {dist:.1f}m"
    cv2.putText(
    img,
    label,
    (x1, max(20, y1 - 10)),
...
def collision_risk(tracks):
    def estimate_speed(track, fps):
        ...
        x1, y1 = track.history[-2]
        x2, y2 = track.history[-1]

        pixel_distance = ((x2 - x1)**2 + (y2 - y1)**2)**0.5

        speed = pixel_distance * fps * 0.05
    ...
    closest = min((t.dist for t in tracks), default=100)
...
def process_video(input_path: str):
    print("[DEBUG] Entered process_video")
    print(BANNER)
    ...
    print("[DEBUG] Loading Depth Model...")
    depth_est = DepthEstimator()
    ocr_reader = easyocr.Reader(['en'])
    print("[DEBUG] Depth Model loaded")
    # ── Setup output writer ───────────────────────────────────────────────────
    ...
    while True:
        ...
        # ── Depth estimation ──────────────────────────────────────────────────
        if frame_no % DEPTH_EVERY == 0 or cached_depth is None:
            small = cv2.resize(frame_rgb, (640,384))
        ...
        for t in tracks:
            ...
            speed = estimate_speed(t, src_fps)
            x1, y1, x2, y2 = map(int, t.bbox)

            plate_crop = frame_bgr[y1:y2, x1:x2]

            if plate_crop.size > 0 and t.dist < 20:

                try:
                    if frame_no % 30 == 0:
                        results_ocr = ocr_reader.readtext(plate_crop)
                    else:
                        results_ocr = []
            ...
            #print("TRACK:", t.id, "CLASS:", t.cls_id)
        ...
        sx = LEFT_W / frame_bgr.shape[1]
        sy = LEFT_H / frame_bgr.shape[0]

        for t1, t2, dist in close_pairs:
            x1 = int(((t1.bbox[0] + t1.bbox[2]) / 2) * sx)
            y1 = int(((t1.bbox[1] + t1.bbox[3]) / 2) * sy)

            x2 = int(((t2.bbox[0] + t2.bbox[2]) / 2) * sx)
            y2 = int(((t2.bbox[1] + t2.bbox[3]) / 2) * sy)
        ...
        cv2.imshow("Urban Digital Twin", composite)
    ...
    if len(plate_records):
        ...
        print("[DONE] Wrote vehicle_log.xlsx")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("For webcam: python main.py camera")
        sys.exit(1)

    input_video = sys.argv[1]

    if input_video != "camera" and not os.path.isfile(input_video):
        print(f"[ERROR] File not found: {input_video}")
        sys.exit(1)

    try:
        print("[DEBUG] About to call process_video")
